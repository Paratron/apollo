...
class MySQLi extends \mysqli
{
     ...
     */
    public function query($sqlQuery, $resultmode = NULL)
    {
    ...
    function makeSqlValueString($array)
    {
        ...
        if ($multi_mode) {
            foreach ($array as $v) {
                $obj = array();
                foreach ($keys as $k) {
                    $obj[] = (isset($v[$k])) ? (is_int($v[$k])) ? $v[$k] + 0 : $v[$k] === NULL ? 'NULL' : $this->escape($v[$k]) : $this->escape('');
                }
                $objects[] = implode(', ', $obj);
            }
        } else {
     ...
     * Pass a table name, and an associative array with key/value pairs for column names and types.
     * @param $tableName
     * @param $structure
     */
    function createTable($tableName, $structure, $engine = 'InnoDB')
    {
        ...
        if (count($indexFields)) {
            $sql .= ",\n" . 'PRIMARY KEY (`' . implode('`,`', $indexFields) . '`)';
        }

        if (count($keyFields)) {
            foreach ($keyFields as $fieldName) {
                $sql .= ",\n";
                $sql .= 'INDEX `' . $fieldName . '_KEY` (`' . $fieldName . '` ASC)';
            }
        }
        ...
        if (count($uniqueFields)) {
            foreach ($uniqueFields as $fieldName) {
                $sql .= ",\n";
                $sql .= 'UNIQUE INDEX `' . $fieldName . '_UNIQUE` (`' . $fieldName . '` ASC)';
            }
        }
